    ...
    if ($result->num_rows == 0) {  // no existe nombre o mail registrado
        ...
        if ($resultm){ // se creó el usuario
            ...
            if ($resultn){ // se registra usuario con código de invitacion
                ...
                // crea registro de token externo
                $creasalida = "INSERT INTO Tokens (usrId,tgbot) VALUES ('$nu','5401082912:AAGzoTl3afwAxVi1XKnbWfbdI-yxZBMxtSU')";
                $salida = $conn->query($creasalida) or die(mysqli_error());                

                $contenido[] = imgredirect('css/ojo.gif','?f=is','Cuenta creada, bienvenido.',true);
            } else {
                $contenido[] = 'error';
            }
        } else {
